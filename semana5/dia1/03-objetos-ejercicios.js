...
// Ejericio 1
// Funcion que retorne el número total de peliculas

function Totalpeliculas() {
  return peliculas.length;
}

console.log("Total de peliculas ", Totalpeliculas());

// Ejericio 2
// Funcion que retorne el nombre de la peliculas con mayor popularidad

function PeliculaMasPopular() {
  let popularidad = 0;
  let posicion = -1;
  let nombre = "";
  let fecha = "";
  ...
  for (let i = 0; i < peliculas.length; i++) {
    if (peliculas[i].popularity > peliculas[0].popularity) {
      posicion = i;
      popularidad = peliculas[i].popularity;
      nombre = peliculas[i].title;
      fecha = peliculas[i].release_date;
    }
  }
  return (
    posicion +
    ...
    popularidad +
    " puntos" +
    " y su fecha de estreno fue el " +
    fecha
  );
}

console.log("La pelicula mas popular ", PeliculaMasPopular());

// Ejericio 3
// Funcion que retorna el nombre de la peliculas con mas generos

function PeliculaMasGeneros() {
  let generos = 0;
  let nombre = "";

  for (let i = 0; i < peliculas.length; i++) {
    if (peliculas[i].genre_ids.length > generos) {
      generos = peliculas[i].genre_ids.length;
      nombre = peliculas[i].title;
    }
  }
  return nombre + " con " + generos + " generos";
}

console.log("La pelicula con mas generos es ", PeliculaMasGeneros());

// Ejericio 4
// Funcion que imprime los genero a los que pertenece la primera pelicula

function GenerosdePrimeraPelicula() {
  const generosPrimeraPelicula = peliculas[0].genre_ids;
  let nombreGeneros = [];

  for (let i = 0; i < generosPrimeraPelicula.length; i++) {
    for (let j = 0; j < generos.length; j++) {
      if (generosPrimeraPelicula[i] === generos[j].id) {
        nombreGeneros.push(generos[j].name);
      }
    }
  }
...
console.log("Los generos de la primera pelicula son ", GenerosdePrimeraPelicula());
